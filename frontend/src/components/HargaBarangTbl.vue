    ...
    <div>
        <div class="container-fluid">
            ...
            <div class="mb-3 d-flex align-items-center gap-2" style="max-width: 500px;">
                <button @click="tambahHargaBarang()" class="btn btn-primary flex-shrink-0">
                    Tambah Harga Barang
                </button>
            ...
            <table class="table table-bordered table-striped table-hover">
                <thead class="table-secondary">
                    <tr>
                        ...
                        <th class="text-center" @click="sortBy('harga_terbaru')">
                            Harga Terbaru
                            <span v-if="sortKey === 'harga_terbaru'">
                                <template v-if="sortAsc">▲</template>
                                <template v-else>▼</template>
                            </span>
                        </th>
                        <th class="text-center" @click="sortBy('mulai_berlaku')">
                            Tgl Berlaku
                            <span v-if="sortKey === 'mulai_berlaku'">
                                <template v-if="sortAsc">▲</template>
                                <template v-else>▼</template>
                            </span>
                        </th>
                ...
                <tbody>
                    <tr v-for="barang in paginatedData" :key="barang.id">
                        ...
                        <td class="text-center">{{ barang.harga_terbaru }}</td>
                        <td class="text-center">{{ barang.mulai_berlaku }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script setup>
    ...
    function tambahHargaBarang() { // func jika button tambah di klik, pindah ke halaman tambah
        router.push('/harga_barang/tambah')
    }
    ...
    let result = props.barangs
        ...
        if (search.value.trim() !== '') {
            ...
            result = result.filter(
                ...
                String(b.harga_terbaru).includes(keyword) || // <-- convert dulu angka ke string agar bisa di filter
                b.mulai_berlaku.toLowerCase().includes(keyword)
            )
        }
